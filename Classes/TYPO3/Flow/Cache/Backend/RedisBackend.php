 ...
 *  - phpredis with Redis 2.6 support (tested with 92782639b0329ff91658a0602a3d816446a3663d from 2014-01-06)
 *
...
class RedisBackend extends AbstractBackend implements TaggableBackendInterface, IterableBackendInterface, FreezableBackendInterface {
	...
	/**
	 * @var int Cursor used for iterating over cache entries
	 */
	...
	public function __construct(\TYPO3\Flow\Core\ApplicationContext $context, array $options = array(), \Redis $redis = NULL) {
		...
		if (NULL === $redis) {
			...
			if (!$redis->connect($this->hostname, $this->port)) {
				throw new \TYPO3\Flow\Cache\Exception('Could not connect to Redis.', 1391972021);
			}
	 ...
	 *
	 * @param int $lifetime
	 * @internal param int $defaultLifetime Default lifetime of this cache backend in seconds. If NULL is specified, the default lifetime is used. "0" means unlimited liftime.
	 * @return void
	 * @api
	 */
	 ...
	 * @internal param string $hostname Hostname of the Redis server
	 * @api
	 */
	 ...
	 *
	 * @param string $port
	 * @internal param string $port Port of the Redis server
	 * @api
	 */
	 ...
	 *
	 * @param int $database
	 * @internal param int $database Database that will be used
	 * @api
	 */
