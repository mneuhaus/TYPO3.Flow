...
trait SecurityOperationsTrait {
	...
	public function iAmNotAuthenticated() {
		if ($this->isolated === FALSE) {
			...
			$authenticatedRoles = array();
			$authenticatedRoles['TYPO3.Flow:Everybody'] = $this->policyService->getRole('TYPO3.Flow:Everybody');
			if ($this->policyService->hasRole('TYPO3.Flow:Anonymous')) {
				$authenticatedRoles['TYPO3.Flow:Anonymous'] = $this->policyService->getRole('TYPO3.Flow:Anonymous');
			} else {
				$anonymousRole = new \TYPO3\Flow\Security\Policy\Role('TYPO3.Flow:Anonymous');
				$anonymousRole->setAbstract(TRUE);
				$authenticatedRoles['TYPO3.Flow:Anonymous'] = $anonymousRole;
			}

			\TYPO3\Flow\Reflection\ObjectAccess::setProperty($this->securityContext, 'roles', $authenticatedRoles, TRUE);
			\TYPO3\Flow\Reflection\ObjectAccess::setProperty($this->securityContext, 'initialized', TRUE, TRUE);
		}
	}
